...
output_file = "data/microphone_dump.csv"
...
class DataToFile:
    ...
    def write_to_csv(self, times: [int], delays: [datetime], data_values: [Any]):
        ...
        with open(self.path, "a+") as f:
            ...
            #simply write our obtained data array
            s = '' + str(data_values[0])
...
class Connection:
    ...
    async def notification_handler(self, sender: str, data: bytearray):
        # data_list = []
        # for items in data:
        ...
        if len(self.rx_data) >= self.dump_size:
            if self.mode == 'TRAINING':
                ...
                predcol = np.array(self.rx_data[-self.dump_size:]).reshape((1, -1))
                start = datetime.now()
                predres = int(self.inferencemodel.predict(predcol)[0])
                print('Time elapsed (hh:mm:ss.ms) {}'.format(datetime.now() - start))
                byte_to_send = predres.to_bytes(1, 'big')
                await connection.client.write_gatt_char(write_characteristic, byte_to_send)
                print(f"Sent: {predres}")
...
if __name__ == "__main__":
    inferenceModel = None 
    try:
        with open('trainedModel/svc.pkl', 'rb') as modelfile:
            inferenceModel = pickle.load(modelfile)
    except:
    ...
    connection = Connection(
        inferenceModel, 'TESTING', loop, read_characteristic, write_characteristic, data_to_file.write_to_csv
    )
    try:
